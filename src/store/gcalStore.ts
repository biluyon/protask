...
import {
  ...
  eventDays, gcalEnabled, hasValidToken, loadGis, setConnectSuccessHandler,
  type GcalCalendar, type GcalEvent, type EventTiming,
} from '../lib/gcal'
      ...
      set({
        events: prev.map(e => {
          ...
          if (e.allDay) {
            ...
            return { ...e, date: newDate, start: newDate, end: addDay(newDate, span) }
          }
          return {
            ...e,
            date: newDate,
    ...
    updateEvent: async (ev, patch) => {
      ...
      if (patch.timing) {
        ...
        if (t.allDay) {
          ...
          patched.end = addDay(t.endDate || t.startDate, 1)
        } else {
